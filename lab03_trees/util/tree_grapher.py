...
class TreeGrapher:
    ...
    def graph_tree(self, root: Node) -> None:
        """
        Graphs a binary tree
        :param root: Binary tree's root node
        """
        self._get_all_node_coords(root)
        self._make_graph()
        self._show_graph()

    def _get_all_node_coords(self, root: Node) -> None:
        """
        ...
        :param root: Binary tree's root node
        """
        node_coords = [(root.key, 0)]
        node_coords.extend(self._get_node_and_children_coords(root.l_child, 0))
        node_coords.extend(self._get_node_and_children_coords(root.r_child, 0))
    ...
    def _get_line_coords(self) -> List[Tuple[Union[Tuple[int, int]], Tuple[None, None]]]:
        """
